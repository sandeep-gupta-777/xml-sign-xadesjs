...
context("Reading XAdES", function () {
    it("X_AT_SIT_1", function (done) {
        ...
        xades.Verify()
            .then(function (res) { return assert.equal(res, true); })
            .then(done, done);
    });
    it("X_BE_CONN_10", function (done) {
        var xades = new _1.SignedXml(XAdES.Parse(X_BE_CONN_10));
    ...
    it("X_FR_CS_3", function () {
        var xades = QualifyingProperties.LoadXml(X_FR_CS_3);
        assert.equal(xades.Target, "#PRF_2");
        assert.equal(xades.SignedProperties.Id, "PRF_2-SignedProperties");
        var signaturePolicyIdentifier = xades.SignedProperties.SignedSignatureProperties.SignaturePolicyIdentifier;
        assert.equal(!!signaturePolicyIdentifier, true);
        var signaturePolicyId = signaturePolicyIdentifier.SignaturePolicyId;
        ...
        var signerRole = xades.SignedProperties.SignedSignatureProperties.SignerRole;
        assert.equal(signerRole.ClaimedRoles.Count, 1);
        assert.equal(signerRole.ClaimedRoles.Item(0).Value, "Marketing manager");
        var signedDataObjectProperties = xades.SignedProperties.SignedDataObjectProperties;
    ...
    it("X_BG_BOR_1", function () {
        var xades = QualifyingProperties.LoadXml(X_BG_BOR_1);
        ...
        var signedSignatureProperties = xades.SignedProperties.SignedSignatureProperties;
        assert.equal(!!signedSignatureProperties, true);
        var cert = signedSignatureProperties.SigningCertificate.Item(0);
        ...
        var signaturePolicyIdentifier = signedSignatureProperties.SignaturePolicyIdentifier;
        assert.equal(!!signaturePolicyIdentifier, true);
        assert.equal(signaturePolicyIdentifier.SignaturePolicyImplied, true);
        var signatureProductionPlace = signedSignatureProperties.SignatureProductionPlace;
    ...
    it("X_BE_CONN_26", function () {
        var xades = QualifyingProperties.LoadXml(X_BE_CONN_26);
        ...
        xades.UnsignedProperties.UnsignedSignatureProperties.Some(function (item) {
    ...
    it("X_IT_ICB_6", function () {
        var xades = QualifyingProperties.LoadXml(X_IT_ICB_6);
