import { APPLICATION_XML, Convert, NamespaceManager, Parse, Select, SelectNamespaces, XE, XmlAttribute, XmlBase64Converter, XmlChildElement, XmlCollection, XmlElement, XmlError, XmlNodeType, XmlNumberConverter, XmlObject, assign } from 'xml-core';
import * as XmlCore from 'xml-core';
import { __decorate } from 'tslib';
import * as tslib_1 from 'tslib';
import { Certificate, CryptoEngine, getCrypto, setEngine } from 'pkijs';
...
class XmlCanonicalizer {
    constructor(withComments, excC14N, propagatedNamespaces = new NamespaceManager()) {
...
class X509Certificate {
    ...
    LoadRaw(rawData) {
        ...
        const asn1 = fromBER(this.raw.buffer);
...
class SignedXml {
    ...
    Sign(algorithm, key, data, options) {
        ...
        return Promise.resolve()
            .then(() => {
            const signingAlg = assign({}, key.algorithm, algorithm);
            ...
            if (RSA_PSS.toUpperCase() === algorithm.name.toUpperCase()) {
                // Add RSA-PSS params
                const alg2 = assign({}, key.algorithm, algorithm);
                    ...
                    if (this.XmlSignature.ObjectList) {
                        this.XmlSignature.ObjectList.Some((obj) => {
                            ...
                            if (found) {
                                ...
                                if (this.Parent) {
                                    const parent = (this.Parent instanceof XmlObject)
                                        ? this.Parent.GetXml()
                                        : this.Parent;
    ...
    TransformSignedInfo(data) {
        ...
        if (this.Parent) {
            ...
            const parentXml = (this.Parent instanceof XmlObject)
                ? this.Parent.GetXml()
                : this.Parent;
            if (parentXml) {
        ...
        return Promise.resolve()
            ...
            if (options.x509) {
                ...
                options.x509.forEach((x509) => {
                    const raw = Convert.FromBase64(x509);
...
export { Select, Parse, Application, XmlCanonicalizerState, XmlCanonicalizer, CryptoConfig, XmlSignature, XmlSignatureObject, XmlSignatureCollection, CanonicalizationMethod, DataObject, DataObjects, DigestMethod, KeyInfo, Reference, References, Signature, SignatureMethodOther, SignatureMethod, SignedInfo, XmlDsigBase64Transform, XmlDsigC14NTransform, XmlDsigC14NWithCommentsTransform, XmlDsigEnvelopedSignatureTransform, XmlDsigExcC14NTransform, XmlDsigExcC14NWithCommentsTransform, Transform, Transforms, X509Certificate, KeyInfoClause, KeyValue, EcdsaPublicKey, NamedCurve, DomainParameters, EcdsaKeyValue, RsaKeyValue, MaskGenerationFunction, PssAlgorithmParams, X509IncludeOption, X509IssuerSerial, KeyInfoX509Data, SPKIData, SelectRootNamespaces, SignedXml };
